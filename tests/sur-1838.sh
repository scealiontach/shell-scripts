#!/usr/bin/env bash
# SUR-1838: pagerduty::incident_data must produce parseable JSON regardless
...
payload=$(incident_data "SVC1" "incident" "$nasty_title" "")
...
keyed=$(incident_data "SVC1" "incident" "T" "abc-123")
...
inject=$(incident_data "SVC1" "incident" "$inject_title" "")
has_priority=$(echo "$inject" | jq '.incident | has("priority")')
assert_eq "false" "$has_priority" "title cannot inject sibling fields" ||
  failures=$((failures + 1))

# 6. incident_data is defined exactly once at file scope (not redefined inside
#    send_incident based on a runtime branch). declare -F should report a
#    single function with no per-call rewrite needed.
declare -F incident_data >/dev/null || {
  echo "FAIL: incident_data is not defined" >&2
  failures=$((failures + 1))
}
